...
#字典转模型注意事项
模型中的Key名字要和字典中的Key一模一样。最好的模型里写上`-(void)setValue:(id)value forUndefinedKey:(NSString *)key`。当然可以用第三方框架[MJExtension](https://github.com/CoderMJLee/MJExtension)实现模型和字典互转。
